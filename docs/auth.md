...
| El dominio se valida en tres capas | Ninguna alcanza sola: `hd` es quitable desde el navegador, el hook solo corre al crear el usuario, el callback cubre a quien ya existía. |
...
| Publishable key, no la `anon` legacy | Ambas son públicas; la legacy expira a fines de 2026. |
